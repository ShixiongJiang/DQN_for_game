...
def self_blood_count(self_gray):
    ...
    for self_bd_num in self_gray[469]:
        ...
        # 血量灰度值80~98
        # print(self_bd_num)
        if self_bd_num > 77 and self_bd_num < 85:
            self_blood += 1
    return self_blood
...
def boss_blood_count(boss_gray):
    ...
    for boss_bd_num in boss_gray[0]:
        ...
        # 血量灰度值65~75
        #print(boss_bd_num)
        if boss_bd_num > 55 and boss_bd_num < 70:
            boss_blood += 1
    return boss_blood

def boss_posture_count(b_gray):
    boss_posture = 0
    for boss_pos_num in b_gray[0]:
        #print(boss_pos_num)
        if boss_pos_num > 100 and boss_pos_num < 155:
            boss_posture += 1
    return boss_posture
...
def action_judge(boss_blood, next_boss_blood,boss_posture,next_boss_posture,self_blood, next_self_blood, stop, emergence_break):
    # get action reward
    ...
    if next_self_blood < 3:     # self dead
        ...
        if next_boss_posture - boss_posture > 10:
            reward = 5
            done = 0
            stop = 0
            emergence_break = 0
...
if __name__ == '__main__':
    ...
    for episode in range(EPISODES):
        ...
        posture_window_gray = cv2.cvtColor(grab_screen(posture_window), cv2.COLOR_BGR2GRAY)
        ...
        boss_posture = boss_posture_count(posture_window_gray)
        # count init blood
        target_step = 0
        # used to update target Q network
        done = 0
        ...
        while True:
            ...
            # collect blood gray graph for count self and boss blood
            posture_window_gray = cv2.cvtColor(grab_screen(posture_window), cv2.COLOR_BGR2GRAY)
            # collect posture gray graph for count boss posture number
            ...
            next_boss_posture = boss_posture_count(posture_window_gray)
            reward, done, stop, emergence_break = action_judge(boss_blood, next_boss_blood,boss_posture,next_boss_posture,
            ...
            self_blood = next_self_blood
            boss_posture = next_boss_posture
